...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	public void mouseMoved(MouseEvent e) {
		int mouseX = e.getX();
		int mouseY = e.getY();
		int mouseColor = ((BufferedImage) maze).getRGB(mouseX, mouseY);
...
//		System.out.println(mouseColor);
		...
		if(mouseColor == endColor){
			...
			showScaryImage("winnerWinnerChickenDinner.jpg");
		}
	}
	...
	private void showScaryImage(String imageName) {
		try {
			maze = ImageIO.read(getClass().getResource(imageName)).getScaledInstance(640, 480, Image.SCALE_DEFAULT);
		} catch (Exception e) {
